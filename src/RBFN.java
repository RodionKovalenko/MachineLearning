...
//test
        ...
        System.out.println("Accuracy train:" + Statistics.measureAccuracy(result, trainPreis));
    ...
    public static double[][] interpolationMatrix(double[][] a) {
        ...
        for (int i = 0; i < a[0].length; i++) {
            for (int j = 0; j < numberOfHiddenNeurons; j++) {
                matrixS[i][j] = calculateKernel(s[i], centers[j]);
    ...
    public static void main(String[] args) {
        ...
        double[][] input = new double[][]{NetworkFunctions.convertStringToNumbers(Data.mengen[15])};
        double[][] target = new double[][]{NetworkFunctions.convertStringToNumbers(Data.preise[15])};

        double[][] menge = {{ 2,4,6,8,10,19, 40, 14, 20, 12, 19, 6}};
        double[][] preis = {{ 4, 7, 10, 20, 14, 34, 50, 65, 6, 1, 13, 8}};
